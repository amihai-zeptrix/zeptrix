...
function renderTenantForm() {
  ...
  return `<div class="modal-layer center"><form class="modal" data-tenant-form><header class="modal-head"><div><h2>${tenant.id ? "Edit tenant" : "Create tenant"}</h2><p class="subcopy">${tenant.id ? "Change tenant details, login access, and billing metadata." : "Provision a workspace, owner, and empty CRM data set."}</p></div><button type="button" class="close-button" data-action="close">×</button></header><div class="form-grid">${formField("Tenant name", "name", tenant.name, "text", true)}${formField("Workspace URL slug", "slug", tenant.slug, "text", true)}${selectField("Plan", "plan", ["Starter", "Growth", "Enterprise"], tenant.plan)}${selectField("Status", "status", ["Active", "Trial", "Suspended"], tenant.status)}${selectField("Region", "region", ["US-East", "EU-West", "AP-South"], tenant.region)}${formField("Seats", "seats", tenant.seats, "number", true)}${formField("Tenant admin login email", "ownerEmail", ownerEmail, "email", true, "full")}${formField("Billing email", "billingEmail", tenant.billingEmail, "email", true, "full")}</div>${ui.authError ? `<p class="error">${escapeHtml(ui.authError)}</p>` : ""}<div class="form-actions"><button type="button" class="button" data-action="close">Cancel</button><button class="button primary">${tenant.id ? "Save tenant" : "Create tenant"}</button></div></form></div>`;
}
