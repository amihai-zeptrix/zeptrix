...
test("CRM settings include Connectivity Gmail and LinkedIn controls", () => {
  ...
  assert.match(app, /verifyLinkedinSessionViaApi/);
  ...
  assert.match(server, /const \{ execFile, spawn \} = require\("child_process"\)/);
  ...
  assert.match(server, /async function startLinkedinLoginSession/);
  assert.match(server, /async function verifyLinkedinLoginSession/);
  assert.match(server, /const linkedinLoginSessions = new Map\(\)/);
  ...
  assert.match(server, /Set LINKEDIN_PUPPETEER_ENABLED=1/);
  ...
  assert.equal(packageJson.scripts["linkedin:spike"], "node scripts/linkedin-puppeteer-spike.js");
  assert.match(packageJson.scripts.check, /scripts\/linkedin-puppeteer-spike\.js/);
  assert.match(packageJson.dependencies["puppeteer-core"], /\^25\.1\.0/);
