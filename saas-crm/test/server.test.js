...
test("CRM settings include Connectivity Gmail and LinkedIn controls", () => {
  ...
  const renderLinkedinSource = functionSource(app, "renderLinkedinIntegrationSettings", "renderInstagramIntegrationSettings");
  const renderInstagramSource = functionSource(app, "renderInstagramIntegrationSettings", "renderGmailDiscoveryPagination");
  ...
  assert.match(renderSettingsPageSource, /const connectivityActive = \["gmail", "linkedin", "instagram", "zoom", "wechat"\]\.includes\(ui\.settingsTab\)/);
  ...
  assert.doesNotMatch(renderSettingsPageSource, /data-settings-tab="instagram">Instagram/);
  ...
  assert.match(renderInstagramSource, /data-instagram-settings-form/);
  assert.match(renderInstagramSource, /<h3>Instagram<\/h3>/);
  assert.match(renderInstagramSource, /never asks for or stores an Instagram password/);
  assert.match(renderInstagramSource, /No Instagram password is stored in the CRM/);
  assert.match(renderInstagramSource, /data-action="connect-instagram"/);
  assert.match(renderInstagramSource, /Connect Instagram/);
  assert.match(renderInstagramSource, /data-action="reconcile-instagram"/);
  assert.match(renderInstagramSource, /data-action="scan-instagram"/);
  assert.match(renderInstagramSource, /Scan messages/);
  assert.match(renderInstagramSource, /Instagram scan results/);
  assert.match(renderInstagramSource, /Contacts and messages found in Instagram/);
  assert.match(renderInstagramSource, /instagramScannedMessages\(tenant\)/);
  assert.doesNotMatch(renderInstagramSource, /type="password"/i);
  assert.doesNotMatch(renderInstagramSource, /name="(?:password|token|cookie|sessionCookie)"/i);
  ...
  assert.match(app, /saveInstagramSettingsViaApi/);
  assert.match(app, /connectInstagramViaApi/);
  assert.match(app, /reconcileInstagramViaApi/);
  assert.match(app, /syncInstagramViaApi/);
  assert.match(app, /params\.get\("instagram"\) === "connected"/);
  assert.match(app, /function instagramIntegration/);
  assert.match(app, /function instagramScannedMessages/);
  assert.match(app, /item\.source === "linkedin"/);
  assert.match(app, /item\.source === "instagram"/);
  ...
  assert.match(server, /instagram_integrations/);
  ...
  assert.match(server, /async function createInstagramHostedAuthLink/);
  assert.match(server, /providers: \["INSTAGRAM"\]/);
  assert.match(server, /\/api\/instagram\/unipile\/callback/);
  assert.match(server, /notify_url/);
  assert.match(server, /async function reconcileLinkedinProviderAccount/);
  assert.match(server, /async function reconcileInstagramProviderAccount/);
  ...
  assert.match(server, /async function syncInstagramMessages/);
  ...
  assert.match(server, /\$\{channelLabel\} import needs account match/);
  assert.match(server, /source: "instagram", channelLabel: "Instagram"/);
  assert.match(server, /pathname === "\/api\/linkedin\/unipile\/callback"/);
  assert.match(server, /pathname === "\/api\/instagram\/unipile\/callback"/);
  ...
  assert.match(server, /pathname\.endsWith\("\/instagram\/connect"\)/);
  assert.match(server, /pathname\.endsWith\("\/instagram\/reconcile"\)/);
  assert.match(server, /pathname\.endsWith\("\/instagram\/sync"\)/);
  assert.match(server, /pathname\.endsWith\("\/instagram\/scan"\)/);
  assert.match(server, /operation: "connect-linkedin"/);
  assert.match(server, /operation: "sync-linkedin"/);
  assert.match(server, /operation: "connect-instagram"/);
  assert.match(server, /operation: "scan-instagram"/);
  ...
  assert.match(schema, /create table instagram_integrations/);
  ...
  assert.match(clickHandlerSource, /action === "connect-instagram"/);
  assert.match(clickHandlerSource, /connectInstagramViaApi\(tenant\.id\)/);
  assert.match(clickHandlerSource, /action === "reconcile-instagram"/);
  assert.match(clickHandlerSource, /finalizeInstagramConnection\(\)/);
  assert.match(clickHandlerSource, /action === "scan-instagram"/);
  assert.match(clickHandlerSource, /syncInstagramViaApi\(tenant\.id, \{ limit: 50 \}\)/);
  assert.match(clickHandlerSource, /Instagram scan completed/);
  ...
  assert.match(submitHandlerSource, /data-instagram-settings-form/);
  assert.match(submitHandlerSource, /saveInstagramSettingsViaApi\(tenant\.id, values\)/);
