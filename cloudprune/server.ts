...
const { adminGrowthCsv, adminGrowthOverview, createGrowthExperiment, recordGrowthEvent, updateGrowthExperiment } = require("./src/growth-service");
...
async function handleApi(req: IncomingMessage, res: ServerResponse, requestUrl: URL): Promise<void> {
  ...
  try {
    ...
    const adminGrowthExperimentMatch = apiPath.match(/^\/api\/admin\/growth\/experiments\/([0-9a-f-]{36})$/i);
    if (req.method === "POST" && adminGrowthExperimentMatch) {
      return json(res, 200, { experiment: await updateGrowthExperiment(req, adminGrowthExperimentMatch[1], await readJson(req)) });
    }
