...
function faqSection(item) {
  ...
  return `      <section class="panel faq">
        ...
        ${item.faqs.map((faq) => `<details><summary>${escapeHtml(faq.question)}</summary><p>${escapeHtml(faq.answer)}</p></details>`).join("")}
      </section>
`;
}

function faqStructuredData(item, canonicalUrl) {
  if (!Array.isArray(item.faqs) || !item.faqs.length) return null;
  return {
  ...
  <body>
    <header class="resource-hero">
      ...
      <div class="trust-strip" aria-label="CloudPrune scan safety">
        ...
        <span>No AWS changes made</span>
        <span>Impact and rollback notes</span>
      </div>
    </header>
    <main>
      <section class="panel">
